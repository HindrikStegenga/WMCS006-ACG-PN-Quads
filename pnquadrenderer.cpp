...
void PnQuadRenderer::init(QOpenGLFunctions_4_1_Core* f, Settings* s) {
    gl = f;
    settings = s;
...
void PnQuadRenderer::initShaders() {
    shaderProg.create();
    shaderProg.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/shaders/tessVertShader.glsl");
    shaderProg.addShaderFromSourceFile(QOpenGLShader::TessellationControl, ":/shaders/pnquadControlShader.glsl");
    shaderProg.addShaderFromSourceFile(QOpenGLShader::TessellationEvaluation, ":/shaders/pnquadEvaluationShader.glsl");
...
void PnQuadRenderer::updateBuffers(Mesh& currentMesh) {
    ...
    meshIBOSize = pnQuadTessPatchIndices.size();
}


void PnQuadRenderer::draw(bool limit) {
    GLuint tessVao;
    if (limit) {
        tessVao = limitVao;
    } else {
